package br.com.envio_email_mailHog.service;
...
import br.com.envio_email_mailHog.util.LogUtil;
...
public class EmailService extends Thread{
	...
	public void enviar(List<Email> emails) {
		this.emails = emails;
		send();
		System.out.println("send lista de emails");
	}
	...
	public void run() {
		...
		for (Email email : emails) {
			...
			if(email.getDestinatario().contains("/")) {
				...
				message.addRecipients(Message.RecipientType.TO, emails.toArray(new InternetAddress[0]));
			} else {
